...
void output_active_gpio(uint32_t first_set, uint32_t second_set)
{
...
void check_active_gpio(uint32_t* two_buffer)
{
...
*(dma_mem+dma_num*0x100) = (1 << 31);//may need to do masking stuff here
...
*(dma_mem+1+dma_num*0x100) = first_cb_address;
*(dma_mem+dma_num*0x100) = 0b1;//may need to do masking stuff here

}

void pwm_dma(uint8_t on_block,uint8_t num_blocks_f,uint32_t dma_num)//rate should be a number between 50-100 that is % of time on
{
...
uint32_t num_control_blocks = num_blocks_f;


uint32_t all_on = 0xffffffff;//0b00001001001001001001001001001001;
uint32_t all_off = 0xffffffff;//0;
...
uint32_t bus_address_on = (uint32_t)source_info->bus_addr;
block_one[1] = bus_address_on;

uint32_t bus_address_off = (uint32_t)source_info2->bus_addr;
